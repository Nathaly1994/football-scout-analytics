...
def radar_comparison(players: list, categories: list) -> go.Figure:
    """Radar original. El mejor jugador lleva estrella en leyenda."""
    if not players:
    ...
    for i, player in enumerate(players[:3]):
        ...
        color = palette[i]
        nombre = player["name"] + " " if i == mejor_idx else player["name"]
        r, g, b = int(color[1:3], 16), int(color[3:5], 16), int(color[5:7], 16)
        fig.add_trace(go.Scatterpolar(
            r=vals, theta=cats,
            fill="toself",
            name=nombre,
            line=dict(color=color, width=3 if i == mejor_idx else 2),
    ...
    fig.update_layout(
        **DARK_LAYOUT,
        ...
        title=dict(text="Perfil Técnico — Mejor jugador con ", font=dict(size=16)),
        legend=dict(bgcolor=COLORS["surface"], font=dict(color=COLORS["text"])),
    )
    return fig
...
def bar_skill_comparison(players_data: list, skill_cols: list) -> go.Figure:
    """
    En CADA habilidad: la barra más alta = color propio BRILLANTE con número.
    Las demás barras = su color propio pero OPACO (35%).
    """
    if not players_data:
        return _empty_fig("Selecciona jugadores para comparar")
    ...
    # Color propio de cada jugador
    colores_jugadores = [
        COLORS["primary"],    # Jugador 1  Azul
        COLORS["secondary"],  # Jugador 2  Verde
        COLORS["highlight"],  # Jugador 3  Dorado
    ]
    ...
    for i, p in enumerate(players_data[:3]):
        vals = all_vals[i]
        color_propio = colores_jugadores[i]

        # Si gana  color brillante opacidad 1.0
        # Si pierde  mismo color pero opacidad 0.25
        bar_opacity = [
            1.0 if ganadores[col_idx] == i else 0.25
            for col_idx in range(len(skill_cols))
        ]
        textos = [
        ...
        fig.add_trace(go.Bar(
            ...
            x=skill_cols,
            y=vals,
            marker=dict(color=color_propio, opacity=bar_opacity),
            text=textos,
            textposition="outside",
            textfont=dict(color=color_propio, size=12),
        ))
...
def gems_by_position(df: pd.DataFrame) -> go.Figure:
    if df is None or df.empty:
        return _empty_fig("Sin datos de posiciones")

    pos_count = df.groupby("primary_position").agg(
        joyas=("short_name", "count"),
        ahorro_avg=("undervalued_pct", "mean")
    ).reset_index().sort_values("joyas", ascending=True)

    n = len(pos_count)
    colors = [
        COLORS["highlight"] if i == n - 1 else COLORS["gray_line"]
        for i in range(n)
    ]
    opacities = [
        1.0 if i == n - 1 else max(0.3, 0.5 + i * 0.02)
        for i in range(n)
    ]

    fig = _base_fig()
    fig.add_trace(go.Bar(
        y=pos_count["primary_position"],
        x=pos_count["joyas"],
        orientation="h",
        marker=dict(color=colors, opacity=opacities),
        text=pos_count["joyas"].apply(lambda v: f"{v} joyas"),
        textposition="outside",
        textfont=dict(
            color=[COLORS["highlight"] if i == n - 1 else COLORS["gray_text"] for i in range(n)],
            size=11,
        ),
        customdata=pos_count["ahorro_avg"],
        hovertemplate="<b>%{y}</b><br>Joyas encontradas: %{x}<br>Infravaloración promedio: %{customdata:.0f}%<extra></extra>",
    ))

    fig.update_layout(
        title=dict(text="Joyas por Posición — Mayor oportunidad en dorado", font=dict(size=16)),
        xaxis_title="Cantidad de Joyas Ocultas",
